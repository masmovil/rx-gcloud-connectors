...
public class PubSubServiceImpl implements PubSubService {
  ...
  public static final int ACK_DEADLINE_SECONDS = 180;
  ...
  PubSubServiceImpl(String projectId, String topicId, String subscriptionId) {
    ...
    createTopicIfNotExists(topicId)
        .andThen(createSubscriptionIfNotExists(subscriptionId))
        .subscribe();
  ...
  private Completable createSubscriptionIfNotExists(String subscriptionId) {
    return Completable.fromAction(() -> {
      try (SubscriptionAdminClient subscriptionAdminClient = SubscriptionAdminClient.create()) {
        log.info("Attempting to create pubsub subscription " + subscriptionId);
        PushConfig pushConfig = PushConfig.newBuilder().build();
        subscriptionAdminClient.createSubscription(projectSubscriptionName, topicName, pushConfig
            , ACK_DEADLINE_SECONDS);
        log.info("Created subscription " + subscriptionId);
      } catch (Exception e) {
        log.info("Could not create subscription " + subscriptionId + ". Already exists?");
        log.info(e.getMessage());
      }
    });
  }
  ...
  @Override
  public <T> PubSubSubscriberImpl<T> addSubscriber(Consumer<T> consumer, Class<T> tClass,
      Vertx vertx) throws SubscriptionCreationException {
    try {
